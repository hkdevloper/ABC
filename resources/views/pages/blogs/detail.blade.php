        ...
        <div class="w-full mb-10 lg:mb-0 overflow-hidden px-2">
            <div class="my-2 md:px-6">
                ...
                <div class="mb-6 flex items-center justify-start">
                    <img src="{{ url('storage/' . $blog->user->company->logo) }}" class="mr-2 w-[80px] h-[80px] rounded-full" alt="avatar" loading="lazy" />
                    <div>
                        <span class="text-sm md:text-base"> Published <u>{{$blog->created_at->diffForHumans()}}</u> by </span>
                        <a href="{{ route('view.company', [$blog->user->company->slug]) }}" class="font-medium text-sm md:text-base">{{$blog->user->company->name}}</a>
                    </div>
                </div>
