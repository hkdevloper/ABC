    ...
    <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12 xxl:col-span-9 grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 mt-8">
                ...
                <form action="{{route('edit.deal', [$deal->id])}}" method="post" enctype="multipart/form-data"
                    ...
                    <div style="display: flex">
                        <div style="margin-right: 50px">
                            ...
                            <br>
                            <input {{$deal->is_active ? "checked" : ""}} type="checkbox" checked
                    ...
                    <div class="mt-3">
                        ...
                        <select required class="select2 input w-full border mt-2" name="user">
                            ...
                            @foreach($users as $user)
                                <option
                                    {{$user->id == $deal->user_id  ? "selected" : ""}} value="{{$user->id}}">{{$user->first_name}} {{$user->last_name}}
                                    [Id: {{$user->id}}]
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-3">
                        ...
                        <select required class="select2 input w-full border mt-2" name="category">
                            ...
                            @foreach($categories as $category)
                                <option
                                    {{$category->id == $deal->category_id  ? "selected" : ""}} value="{{$category->id}}">{{$category->name}}
                                    [Id: {{$category->id}}]
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-3">
                        ...
                        <input id="name" type="text" class="input w-full border mt-2"
                               ...
                               value="{{old('title') ?old('title') : $deal->title}}">
                    </div>
                    <div class="mt-3">
                        ...
                        <input name="slug" type="text" class="input w-full border mt-2 flex-1" required
                               placeholder="AAA>>bb>>c" id="slug" value="{{old('slug') ?old('slug') : $deal->slug}}">
                    </div>
                    <div class="mt-3">
                        <label>Description</label>
                        <textarea id="editor" class="input w-full border mt-2" name="description"
                                  placeholder="Enter Category Description here">{{old('description') ?old('description') : $deal->description}}</textarea>
                    </div>
                    {{--Opening date-time --}}
                    <div class="mt-3">
                        <label>Offer start Date-Time</label>
                        <input type="datetime-local" required class="input w-full border mt-2" name="start"
                               value="{{old('start') ?old('start') : $deal->offer_start_date}}"
                    ...
                    <div class="mt-3">
                        ...
                        <input type="text" class="input w-full border mt-2" required name="discount"
                               value="{{old('discount') ?old('discount') : $deal->discount_value}}"
                               placeholder="Enter Discount here">
                    </div>
                    <div class="mt-3">
                        <label>Discount Code</label>
                        <input type="text" class="input w-full border mt-2" required name="code"
                               value="{{old('code') ?old('code') : $deal->discount_code}}"
                    ...
                    <div class="mt-3">
                        ...
                        <textarea id="editor_terms" class="editor input w-full border mt-2" name="tos" required
                                  placeholder="Enter Category Description here">{{old('tos') ?old('tos') : $deal->terms_and_conditions	}}</textarea>
                    </div>
                    <div class="mt-3">
                        ...
                        <input name="meta_title" type="text" class="input w-full border mt-2 flex-1" required
                               placeholder="" value="{{old('meta_title') ?old('meta_title') : $seo->title}}">
                    </div>
                    <div class="mt-3">
                        ...
                        <input name="meta_keywords" type="text" class="input w-full border mt-2 flex-1" required
                               ...
                               value="{{old('meta_keywords') ?old('meta_keywords') : $seo->meta_keywords}}">
                    </div>
                    <div class="mt-3">
                        ...
                        <input name="meta_description" type="text" class="input w-full border mt-2 flex-1"
                               required
                               placeholder=""
                               value="{{old('meta_description') ?old('meta_description') : $seo->meta_description}}">
                    </div>
